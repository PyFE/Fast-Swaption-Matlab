...
for k = 1 : length(optMaturity)
    ...
    for j = 1 : length(swapMaturity)
        ...
        price = sp_swaption(swaption, model3, crv)*10000;
        exac = lgm_swaption(swaption, model3, crv)*10000;
        vol = sp_swaption(swaption, model3, crv, 'output', 'vol');
        price_ATM(k,j)=price;
        error_ATM(k,j)=price-exac;
        % In the money
